...
@app.route('/organizations/<string:id>', methods=['GET'])
...
@app.route('/organizations/<string:id>', methods=['PUT'])
...
@app.route('/organizations/<string:id>', methods=['DELETE'])
...
@app.route('/organizations/<string:id>/branches', methods=['GET'])
def get_branches(id):
    connection = create_connection()
    cursor = connection.cursor(dictionary=True)
...
@app.route('/organizations/<string:orgid>/branches', methods=['POST'])
def add_branch(orgid):
    data = request.json
    id = str(uuid.uuid4())
    connection = create_connection()
    cursor = connection.cursor()
    cursor.execute("INSERT INTO branch (id, name, organization_id) VALUES (%s, %s, %s)", (id, data['name'], orgid))
    connection.commit()
    branch_id = cursor.lastrowid
    connection.close()
    return jsonify({'id': branch_id, 'name': data['name']})

@app.route('/branches/<string:id>', methods=['GET'])
def get_branch(id):
    try:
        connection = create_connection()
        cursor = connection.cursor(dictionary=True)
...
@app.route('/branches/<string:id>', methods=['PUT'])
def update_branch(id):
    ...
    try:
        connection = create_connection()
        cursor = connection.cursor()
        cursor.execute("UPDATE branch SET name = %s WHERE id = %s", (name, id))
        connection.commit()
        if cursor.rowcount > 0:
...
@app.route('/branches/<string:id>', methods=['DELETE'])
...
@app.route('/branches/<string:branch_id>/departments', methods=['GET'])
...
@app.route('/branches/<string:branch_id>/departments', methods=['POST'])
def add_department(branch_id):
    ...
    try:
        connection = create_connection()
        cursor = connection.cursor()
        cursor.execute("INSERT INTO department (id, name, branch_id) VALUES (%s, %s, %s)", (id, data['name'], branch_id))
        connection.commit()
...
def update_user(id):
    ...
    try:
        ...
        cursor.execute("UPDATE user SET username = %s, password = %s, role = %s, access_id = %s WHERE id = %s",
                       (username, hashed_password, role, access_id, id))
        connection.commit()
        if cursor.rowcount > 0:
